...
\author{%
  MENG GUO \quad 48-266606\\
  Department of Creative Informatics,
  Graduate School of Information Science and Technology\\
  The University of Tokyo
  \quad$\vert$\quad Laboratory: \textit{$\langle$TODO$\rangle$}\\
  Own research topic: \textit{$\langle$TODO$\rangle$}\\[2pt]
  \small Code: \url{https://github.com/mengguo311/sc-gs-failure-analysis}}
...
\begin{abstract}
\noindent
Sparse-Controlled Gaussian Splatting (SC-GS) drives dense 3D Gaussians with a
few hundred learned control points, making a dynamic scene both renderable from
novel views and editable through as-rigid-as-possible (ARAP) deformation of the
control graph. We ask when that editing breaks. Its default mode initializes
every ARAP solve from a linear Laplacian solution, which cannot represent
rotation; with a fixed budget of three local-global iterations it never
recovers, and rotational drags beyond 45--60$^\circ$ collapse into
under-rotated, shortened limbs and torn Gaussian layouts. We identify two
further failure axes: an initialization-independent artifact in the rotation
re-estimation that transfers node motion to the Gaussians, and a sensitivity of
\emph{editability} --- but not reconstruction quality --- to control-point
count, including a global ``leakage'' failure that local rigidity metrics
cannot detect. Our remedy, \emph{progressive drag scheduling}, subdivides a drag
into $N$ sub-steps and warm-starts each solve from the previous one. Retraining
nothing and touching no solver code, it moves the failure onset to
90--110$^\circ$ ($N{=}8$) at under 0.8\,s per drag --- 16$\times$ cheaper than
SC-GS's robust-but-slow alternative --- and closes 43--80\% of the rigidity gap
across eight D-NeRF scenes.

...
\begin{figure*}[t]
  ...
  \caption{\textbf{Original SC-GS vs.\ our method: edge-stretch vs.\ drag
  angle on all eight scenes.} Red = \scgsOrig{}; pink/orange/green = ours with
  growing $N$; blue = the slow reference. The ordering is preserved across
  organic limbs, a mechanical hinge (\texttt{lego}), an extreme kinematic chain
  (\texttt{trex}), and a free body on an arc (\texttt{bouncingballs}); absolute
  levels are scene-dependent.}
...
(Tab.~\ref{tab:crossscene}, Fig.~\ref{fig:crossscene}). At 90$^\circ$ \ours{8}
